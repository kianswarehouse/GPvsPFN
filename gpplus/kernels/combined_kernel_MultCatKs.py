...
class CombinedKernel_MultCatKs(gpytorch.kernels.Kernel):
    def __init__(
        self,
        cont_cols: list = None,
        cat_cols: list = None,
        ...
        **kwargs,
        ...
        """
        Multi-Variable Multi-Fidelity Combined Kernel with Multiple Categorical Kernels
        ...
        Args:
            ...
            cat_kernel: Kernel for categorical features. Can be:
                - None: Creates separate GaussianKernel for each categorical group (default)
                - Single kernel: Used for all groups (backward compatibility)
                - List of kernels: One kernel per categorical group [k1, k2, k3, ...]
    ...
    def forward(self, x1, x2=None, diag=False, **kwargs):
        ...
        if self.cat_kernel is not None and self.cat_encoder is not None:
            ...
            for i, (encoder, col_group, kernel) in enumerate(zip(encoders, self.cat_cols, kernels)):
        ...
        if cat_encoder is not None or cat_kernel is not None or len(self.cat_cols) > 0:
            ...
            for i, cat_group in enumerate(self.cat_cols):
                ...
                if cat_kernel is None:
                    ...
                    gauss_k = GaussianKernel(ard_num_dims=encoder.z_dim)
                    gauss_k.raw_lengthscale.requires_grad_(True)
                    # gauss_k.raw_lengthscale.data = torch.ones(encoder.z_dim) * 0.0
                    temp_cat_kernel.append(gauss_k)
                elif isinstance(cat_kernel, list):
                    # Use the corresponding kernel from the list
                    if i < len(cat_kernel):
                        temp_cat_kernel.append(cat_kernel[i])
                    else:
                        raise ValueError("Numbers of kernels provided does not match number of categorical groups")
                else:
                    # Single kernel provided - use it for all groups (backward compatibility)
                    # Note: This assumes all encoders have the same z_dim
                    if i == 0:
