...
def griewank_GPvsPFN(num_runs=defaults.NUM_RUNS,
        ...
        # n_jobs=1, # Only use one thread for training
        # inner_max_num_threads=1, # Only use one thread for inner training
...
if __name__ == "__main__":
    ...
    for dim_val in (10, 40):
        for train_mult in (10, 40):
            for noise in (0.005, 0.05):
                griewank_GPvsPFN(
                    num_runs=defaults.NUM_RUNS,
                    train_size=train_mult,
                    dimensions=dim_val,
                    num_test=5000,
                    noise_train=noise,
                    noise_test=noise,
