...
def generate_mf_buckling_data_with_folds(train_samples_per_source: list[int], test_samples_per_source: list[int], 
    ...
    for src_idx, (src, n_train) in enumerate(zip(sources, train_samples_per_source)):
        ...
        # Generate continuous variables using Sobol
        sobol_seed = (seed + src_idx * 2000) if seed is not None else None
        sobol = torch.quasirandom.SobolEngine(1, scramble=True, seed=sobol_seed)
        L_vals = sobol.draw(n_train).squeeze() * 0.4 + 0.6  # L in [0.6, 1.0]
        ...
        # Split into folds with even distributions PER (E,K,I) COMBINATION
        # This guarantees balanced marginals in every fold
        for e_val in E_values:
            for k_val in K_values:
                for i_val in I_values:
                    mask = (
                        (x_train_block[:, 1] == e_val)
                        & (x_train_block[:, 2] == k_val)
                        & (x_train_block[:, 3] == i_val)
                    )
                    combo_indices = torch.where(mask)[0]
                    if combo_indices.numel() == 0:
                        continue
                    # Shuffle within combination to avoid any ordering artifacts
                    combo_indices = combo_indices[torch.randperm(combo_indices.numel())]
                    base = combo_indices.numel() // num_folds
                    rem = combo_indices.numel() % num_folds
                    start = 0
...
def inverse_1D_ex(X: torch.Tensor, source: str = "s0") -> torch.Tensor:
    """
    ...
    base_result = 1 / (0.15 * x1**3 + x1**2 + x1 + 1)
    
    if source == "s0":
        # High fidelity: accurate model
        result = base_result
    elif source == "s1":
        # Medium fidelity: add small bias
        result = base_result + 0.1
    elif source == "s2":
        # Lower fidelity: add larger bias
        result = base_result + 0.2
    elif source == "s3":
        # Even lower fidelity: add even larger bias
        result = base_result + 0.3
    else:
        # For additional sources, add progressive bias
        source_num = int(source[1:])  # Extract number from "sX"
        result = base_result + 0.1 * source_num

    return result


def generate_mf_inverse_1D_data(samples_per_source: list[int], seed: int = 0, noise: list[float] = None, noise_type: str = 'gaussian') -> tuple[torch.Tensor, torch.Tensor]:
    """
    ...
    # Set default noise levels based on number of sources
    num_sources = len(samples_per_source)
    if noise is None:
        noise = [0.0] * num_sources

    # Bounds for the continuous feature (x1)
    l_bound = torch.tensor([-1.0], dtype=torch.float64)
    u_bound = torch.tensor([2.0], dtype=torch.float64)
    ...
    for idx, (src, n) in enumerate(zip(sources, counts)):
        ...
        y = inverse_1D_ex(x_raw, source=src)
        
        # Add noise based on source
        noise_level = noise[idx]
        if noise_level > 0:
            if noise_type == 'gaussian':
                noise = torch.randn_like(y) * noise_level * y.std().item()
            elif noise_type == 'uniform':
                noise = (torch.rand_like(y) - 0.5) * 2 * noise_level * y.std().item()
            else:
                raise ValueError(f"Unknown noise_type: {noise_type}")
            y = y + noise
        
        # Append source class as the 2nd feature   
