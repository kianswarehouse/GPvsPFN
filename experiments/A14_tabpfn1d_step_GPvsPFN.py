...
def tabpfn1d_step_GPvsPFN(
    ...
    if title is None:
        title = f"TabPFN1D_step_{dimensions}Dx_{train_size}Dn_[{x_bounds[0]},{x_bounds[1]}]_{num_inits}inits_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"
    else:
        title = f"TabPFN1D_step_{title}_{dimensions}Dx_{train_size}Dn_[{x_bounds[0]},{x_bounds[1]}]_{num_inits}inits_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"
    ...
    print(
        f"Generating {total_samples} unique Sobol samples for 1D step function\n\tTest samples: {num_test} / Train samples: {total_train}"
    )

    X_train_all, y_train_all, X_test_all, y_test_all = generate_tabpfn_1d_step_data(
        n_train=total_train,
        n_test=num_test,
    ...
    _tb = (float(x_bounds[0]), float(x_bounds[1]))
    y_true_1d = tabpfn_1d_step_function(
        X_test_all.to(dtype=torch.float64), x_bounds=_tb, step_values=step_values
