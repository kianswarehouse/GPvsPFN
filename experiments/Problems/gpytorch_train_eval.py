...
def train_eval_gp_gpytorch_default(
    ...
    for run_idx in range(num_runs):
        ...
        if lr is None:
            if optimizer_class == torch.optim.LBFGS:
                lr = getattr(defaults, "LBFGS_LR", 0.1)
        ...
        for epoch in range(num_epochs):
            ...
            while not epoch_successful and not skip_run:
                with (
                    ...
                    try:
                        ...
                        if torch.isnan(loss) or torch.isinf(loss):
                            print(
                                f"  Run {run_idx + 1}/{num_runs}, Epoch {epoch + 1}/{num_epochs}: NaN/Inf loss detected. Skipping this run."
                            )
                        ...
                        if jitter < max_jitter:
                            ...
                            print(
                                f"  Run {run_idx + 1}/{num_runs}, Epoch {epoch + 1}/{num_epochs}: NotPSDError detected. Increasing jitter to {jitter:.1e}."
                            )
                            # Retry this epoch with higher jitter
                            epoch_successful = False
                        else:
                            print(
                                f"  Run {run_idx + 1}/{num_runs}, Epoch {epoch + 1}/{num_epochs}: NotPSDError persists even with jitter={jitter:.1e}. Skipping this run."
                            )
                            # Skip this run - restore original state and break
                            model.load_state_dict(original_state)
                            skip_run = True
                        ...
                        if "nan" in error_str or "nanerror" in error_str:
                            print(
                                f"  Run {run_idx + 1}/{num_runs}, Epoch {epoch + 1}/{num_epochs}: NaN error detected: {e}. Skipping this run."
                            )
                            model.load_state_dict(original_state)
                            skip_run = True
                            break
                        if (
                            ...
                            if jitter < max_jitter:
                                ...
                                run_jitter = jitter
                                print(
                                    f"  Run {run_idx + 1}/{num_runs}, Epoch {epoch + 1}/{num_epochs}: NotPSD error detected. Increasing jitter to {jitter:.1e}."
                                )
                                epoch_successful = False
                                continue
                            print(
                                f"  Run {run_idx + 1}/{num_runs}, Epoch {epoch + 1}/{num_epochs}: NotPSD error persists even with jitter={jitter:.1e}. Skipping this run."
                            )
                            model.load_state_dict(original_state)
                            skip_run = True
                            break
            ...
            if (epoch + 1) % 1000 == 0 or epoch == 0:
                print(
                    f"  Run {run_idx + 1}/{num_runs}, Epoch {epoch + 1}/{num_epochs}: Loss = {loss_val:.6f}"
                )
    ...
    if best_state_dict is None:
        print(f"  ERROR: All {num_runs} training runs failed due to numerical instability.")
        ...
        gp_metric = {
            ...
            "evaluation_error": f"All {num_runs} training runs failed - no valid model",
