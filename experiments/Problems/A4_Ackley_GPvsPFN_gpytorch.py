...
def ackley_GPvsPFN(num_folds=defaults.NUM_FOLDS,
    ...
    v2 = "V2" if V2 else ""
    if title is None:
        title = f"Ackley{v2}_gpytorch_{dimensions}xdim_{train_size}D_[{x_bounds[0]},{x_bounds[1]}]bounds_noiseTest{noise_test}_noiseTrain{noise_train}"
    else: 
        title = f"Ackley{v2}_gpytorch_{title}_{dimensions}xdim_{train_size}D_[{x_bounds[0]},{x_bounds[1]}]bounds_noiseTest{noise_test}_noiseTrain{noise_train}"
...
if __name__ == "__main__":
    ackley_GPvsPFN(num_folds=1, train_size=20, dimensions=20, num_runs=4, num_epochs=1000, save_path='./results/Ackley/temp')
