...
def wing_SF_GPvsPFN(num_folds=defaults.NUM_FOLDS,
    ...
    if title is None:
        title = f"wing_SF_gpytorch_{train_size}D_{num_runs}runs_noiseTest{noise_test}_noiseTrain{noise_train}"
    else: 
        title = f"wing_SF_gpytorch{title}_{train_size}D_{num_runs}runs_noiseTest{noise_test}_noiseTrain{noise_train}"
...
if __name__ == "__main__":
    wing_SF_GPvsPFN(num_folds=1, lr=0.1, train_size=10, num_test=5000, noise_train=0.0025, noise_test=0.025, num_runs=4, num_epochs=5000, save_path="./results/wing/temp/gpytorch")
