...
from gpplus.utils.encoders import MatrixEncoder, NeuralEncoder as OneHotToLatent
from gpplus.utils.metrics_functions import compute_metrics, compute_per_source_metrics

# from gpplus.likelihood import MultiLikelihood_og as MultiLikelihood
...
for seed in range(42, 52):
    ...
    noise_variances = [0.1**2, 0.5**2, 1.0**2]  # [0.01, 0.25, 1.0]
    prior_mean = np.log(np.mean(noise_variances))
    noise_prior = gpytorch.priors.LogNormalPrior(loc=prior_mean, scale=0.1)
    # noise_prior = gpytorch.priors.LogNormalPrior(loc=prior_mean, scale=1.0)
    from gpytorch.constraints import GreaterThan

    multi_mean = gpplus.means.MultiMean(
        encoded_cols=np.arange(0, 4),
        means=[
            gpytorch.means.ZeroMean(),
            gpytorch.means.ConstantMean(constant_prior=gpytorch.priors.NormalPrior(0.0, 1.0)),
            gpytorch.means.ConstantMean(constant_prior=gpytorch.priors.NormalPrior(0.0, 1.0)),
        ],
    )

    # multi_mean = gpplus.means.MultiMean(encoded_cols=4,
    #             means = [gpytorch.means.ConstantMean(constant_prior=gpytorch.priors.NormalPrior(0.0, 1.0)),
    #                     gpytorch.means.ConstantMean(constant_prior=gpytorch.priors.NormalPrior(0.0, 1.0)),
    #                     gpytorch.means.ConstantMean(constant_prior=gpytorch.priors.NormalPrior(0.0, 1.0))])

    # OH encoding implementation
    multi_likelihood = MultiLikelihood(encoded_cols=X_train[:, np.arange(0, 4)], training_data=X_train)

    # Normal categorical column implementation (assuming source column is in last column)
    # multi_likelihood = MultiLikelihood(encoded_cols=X_train[:,-1], noise_prior=noise_prior)
    ...
    model = GPR(
        ...
        mean_module=multi_mean,  # means = gpytorch.means.ConstantMean(constant_prior=gpytorch.priors.NormalPrior(0.0, 1.0))),
    ...
    # Print initial mean hyperparameters
    print(f"\nInitial mean hyperparameters (Run #{i}):")
    if hasattr(model.mean_module, "means"):
        for j, mean_func in enumerate(model.mean_module.means):
            print(f"  Mean {j} ({type(mean_func).__name__}):")
    ...
    # trainer.train_single_process(0)
    ...
    # Print final mean hyperparameters after training
    print(f"\nFinal mean hyperparameters (Run #{i}):")
    if hasattr(model.mean_module, "means"):
        for j, mean_func in enumerate(model.mean_module.means):
            print(f"  Mean {j} ({type(mean_func).__name__}):")
            # Show all named parameters for each mean function
    ...
    per_source_metrics = compute_per_source_metrics(y_test_std, y_pred_std, output_std_std, X_test, np.arange(0, 4), t1)
