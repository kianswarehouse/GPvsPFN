...
def ackley_GPvsPFN(num_runs=defaults.NUM_RUNS,
        num_test=5000,
    ...
    # v2 = "V2" if V2 else ""
    if title is None:
        title = f"ackley_gpytorch_{dimensions}xdim_{train_size}D_[{x_bounds[0]},{x_bounds[1]}]bounds_noiseTest{noise_test}_noiseTrain{noise_train}"
    else: 
        title = f"ackley_gpytorch_{title}_{dimensions}xdim_{train_size}D_[{x_bounds[0]},{x_bounds[1]}]bounds_noiseTest{noise_test}_noiseTrain{noise_train}"
...
if __name__ == "__main__":
    ackley_GPvsPFN(num_runs=1, train_size=20, dimensions=20, num_inits=4, num_epochs=100, save_path='./results/ackley/temp')
