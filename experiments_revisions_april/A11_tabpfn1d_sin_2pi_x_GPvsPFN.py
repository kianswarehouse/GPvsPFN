...
from load_experimental_data import generate_tabpfn_1d_sin_2pi_x_data, tabpfn_1d_sin_2pi_x_function
from plot_tabpfn1d_comparison import save_1d_train_gp_tabpfn_plot
from tabpfn import TabPFNRegressor

import gpplus
from gpplus.likelihoods import MLPGaussianLikelihood, mlp_noise_head
from gpplus.utils import set_seed, train_eval_gp, train_eval_PFN


def tabpfn1d_sin_2pi_x_GPvsPFN(
        ...
        save_path="./results/TabPFN1D_sin_2pi_x",
        title=None,
        ...
        gp_mlp_heteroskedastic: bool = False,
    ...
    if title is None:
        title = f"TabPFN1D_sin_2pi_x_{dimensions}Dx_{train_size}Dn_[{x_bounds[0]},{x_bounds[1]}]_{num_inits}inits_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"
    else:
        title = f"TabPFN1D_sin_2pi_x_{title}_{dimensions}Dx_{train_size}Dn_[{x_bounds[0]},{x_bounds[1]}]_{num_inits}inits_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"
    ...
    print(
        f"Generating {total_samples} unique Sobol samples for 1D f(x)=sin(2*pi*x)\n\tTest samples: {num_test} / Train samples: {total_train}"
    )

    X_train_all, y_train_all, X_test_all, y_test_all = generate_tabpfn_1d_sin_2pi_x_data(
        n_train=total_train,
        n_test=num_test,
    ...
    y_true_1d = tabpfn_1d_sin_2pi_x_function(X_test_all.to(dtype=torch.float64)).detach().cpu().numpy().ravel()
    ...
    for i in range(num_runs):
        ...
        if run_models in [None, "gp"]:
            ...
            if gp_mlp_heteroskedastic:
                d_in = X_train.shape[1]
                mlp = mlp_noise_head(d_in, hidden_dims=gp_mlp_hidden_dims, dtype=gp_dtype)
                gp_likelihood = MLPGaussianLikelihood(mlp)
            else:
                gp_likelihood = defaults.SF_likelihood
        ...
        if i == 0:
            ...
            if run_models in [None, "gp"]:
                gp_model_info = {
                    ...
                    "gp_mlp_hidden_dims": list(gp_mlp_hidden_dims),
                    "lr": lr,
...
if __name__ == "__main__":
    tabpfn1d_sin_2pi_x_GPvsPFN(
        num_runs=5,
        train_size=20,
        dimensions=1,
        num_inits=4,
        num_epochs=10000,
        save_path="./results/TabPFN1D_sin_2pi_x/test",
        run_models="pfn",
    )
