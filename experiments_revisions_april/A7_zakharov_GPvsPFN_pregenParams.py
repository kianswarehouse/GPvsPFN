...
def zakharov_GPvsPFN_pregenParams(num_runs=defaults.NUM_RUNS,
        num_test=5000,
        ...
        compare_log_point_inverses: bool = True,
        noise_train=0.0,
        noise_test=0.0,
    ...
    if title is None:
        title = f"Zakharov_pregenParams_{dimensions}Dx_{train_size}Dn_[{x_bounds[0]},{x_bounds[1]}]_{num_inits}inits_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"
    else:
        title = f"Zakharov_pregenParams_{title}_{dimensions}Dx_{train_size}Dn_[{x_bounds[0]},{x_bounds[1]}]_{num_inits}inits_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"
    ...
    if run_models in (None, "gp") and num_inits > 0:
        ...
        y_scaled_tm = standardize_y or standardize_y_log_scale
        if single_dataset:
        ...
        if standardize_y_log_scale:
            if log_y_C is not None:
                computed_log_y_C_tm = float(log_y_C)
            else:
                if y_tm.min().item() < 0:
                    computed_log_y_C_tm = float((-y_tm.min()).item() + log_y_epsilon)
                else:
                    computed_log_y_C_tm = float(log_y_epsilon)
            Yscaler_tm = gpplus.utils.LogScaler(epsilon=log_y_epsilon, C=computed_log_y_C_tm)
        else:
        ...
        tmpl = gpplus.models.GPR(
            X_tm,
            y_tm_normal if y_scaled_tm else y_tm,
    ...
    for i in range(num_runs):
        ...
        if run_models in [None, 'gp']:
            ...
            gp_metric_alt = None
            if compare_log_point_inverses and standardize_y_log_scale:
        ...
        if run_models in [None, 'pfn']:
            ...
            if compare_log_point_inverses and standardize_y_log_scale:
                alt_inverse = "median" if log_y_point_inverse == "mean" else "mean"
                tabpfn_metric_alt, _, _ = train_eval_PFN(
                    X_train,
                    X_test,
                    y_train_normal if y_scaled else y_train,
                    y_test,
                ...
                TabPFN_metrics_alt.append(tabpfn_metric_alt)
                print(
                    f"[PFN compare] primary={log_y_point_inverse} RMSE={tabpfn_metric.get('RMSE')} | "
                    f"alt={alt_inverse} RMSE={tabpfn_metric_alt.get('RMSE')}"
                )
                for k in ("RRMSE", "RMSE", "MAE", "CRPS", "NIS", "NCRPS"):
                    if k in tabpfn_metric_alt:
                        tabpfn_metric[f"{k}_{alt_inverse}"] = tabpfn_metric_alt[k]
...
if __name__ == "__main__":
    zakharov_GPvsPFN_pregenParams(single_dataset=False, log_y_C=1, num_runs=1, train_size=20, dimensions=20, num_inits=16, save_path='./results/zakharov/temp', run_models='gp')
    # zakharov_GPvsPFN_pregenParams(num_runs=1, train_size=20, dimensions=20, num_runs=4, save_path='./results/zakharov/temp')
    # zakharov_GPvsPFN_pregenParams(num_runs=1, train_size=40, dimensions=20, num_inits=4, save_path='./results/zakharov/temp')
    # zakharov_GPvsPFN_pregenParams(num_runs=1, train_size=80, dimensions=20, num_inits=4, save_path='./results/zakharov/temp')
    # zakharov_GPvsPFN_pregenParams(num_runs=1, train_size=10, dimensions=40, num_inits=4, save_path='./results/zakharov/temp')
    # zakharov_GPvsPFN_pregenParams(num_runs=1, train_size=20, dimensions=40, num_inits=4, save_path='./results/zakharov/temp')
    # zakharov_GPvsPFN_pregenParams(num_runs=1, train_size=40, dimensions=40, num_inits=4, save_path='./results/zakharov/temp')
    # zakharov_GPvsPFN_pregenParams(num_runs=1, train_size=80, dimensions=40, num_inits=4, save_path='./results/zakharov/temp')
