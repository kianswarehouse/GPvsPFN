...
def tabpfn1d_sin_2pi_x_plus_x_GPvsPFN(
        ...
        save_path="./results/TabPFN1D_sin_2pi_x_plus_x",
        title=None,
    ...
    if title is None:
        title = f"TabPFN1D_sin_2pi_x_plus_x_{dimensions}Dx_{train_size}Dn_[{x_bounds[0]},{x_bounds[1]}]_{num_inits}inits_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"
    else:
        title = f"TabPFN1D_sin_2pi_x_plus_x_{title}_{dimensions}Dx_{train_size}Dn_[{x_bounds[0]},{x_bounds[1]}]_{num_inits}inits_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"
    ...
    print(
        f"Generating {total_samples} unique Sobol samples for 1D f(x)=sin(2*pi*x)+x\n\tTest samples: {num_test} / Train samples: {total_train}"
    )
...
if __name__ == "__main__":
    tabpfn1d_sin_2pi_x_plus_x_GPvsPFN(
        ...
        num_inits=4,
        num_epochs=10000,
        save_path="./results/TabPFN1D_sin_2pi_x_plus_x/test",
        run_models="pfn",
    )
