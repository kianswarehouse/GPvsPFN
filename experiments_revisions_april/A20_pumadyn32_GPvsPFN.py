...
from load_experimental_data import load_pumadyn32_data
import defaults


def pumadyn32_GPvsPFN(
    num_runs=defaults.NUM_RUNS,
    train_size=10,  # total training size is train_size * input dimensions
    train_pool_size=5192,
    test_pool_size=3000,
    ...
    if title is None:
        title = (
            f"puma32H_32Dx_{train_size}Dn_"
            f"{num_inits}inits_split{split_seed}_x{num_runs}"
        )
    else:
        title = (
            f"puma32H_{title}_32Dx_{train_size}Dn_"
            f"{num_inits}inits_split{split_seed}_x{num_runs}"
        )
    ...
    set_seed(seed)
    X_all, y_all = load_pumadyn32_data(print_info=True)
    ...
    # Fixed global split from one randperm.
    ...
    # All features are continuous; do not one-hot (pumadyn has no categoricals).
    qual_dict: dict = {}
    ...
    for i in range(num_runs):
        ...
        if use_disjoint_train_splits:
            ...
            # Independent per-run draw from fixed pool: overlap across runs is allowed.
...
if __name__ == "__main__":
    pumadyn32_GPvsPFN(
        ...
        test_pool_size=3000,
        split_seed=42,
        save_path="./results/pumadyn32/temp",
        run_models="gp",
    )
