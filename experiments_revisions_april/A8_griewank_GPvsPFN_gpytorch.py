...
def griewank_GPvsPFN(num_runs=defaults.NUM_RUNS,
    ...
    # v2 = "V2" if V2 else ""
    if title is None:
        title = f"griewank_gpytorch_{dimensions}xdim_{train_size}D_[{x_bounds[0]},{x_bounds[1]}]bounds_noiseTest{noise_test}_noiseTrain{noise_train}"
    else: 
        title = f"griewank_gpytorch_{title}_{dimensions}xdim_{train_size}D_[{x_bounds[0]},{x_bounds[1]}]bounds_noiseTest{noise_test}_noiseTrain{noise_train}"
...
if __name__ == "__main__":
    griewank_GPvsPFN(num_runs=1, train_size=20, dimensions=20, num_inits=4, num_epochs=100, save_path='./results/griewank/temp')
    # griewank_GPvsPFN(num_runs=1, train_size=10, dimensions=20, num_inits=4, num_epochs=10000, save_path='./results/griewank/temp', standardize_X=False, standardize_y=False)
    # griewank_GPvsPFN(num_runs=1, train_size=10, num_inits=4, num_epochs=10000, save_path='./results/griewank/tempv2', V2=True)
