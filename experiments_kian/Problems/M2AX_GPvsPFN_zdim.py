...
def M2AX_GPvsPFN(
    ...
    if title is None:
        title = f"M2AX_z{z_dim}_{test_size}tsize_{num_epochs}epochs_{num_runs}runs_{lr}"
    else: 
        title = f"M2AX_z{z_dim}_{title}"
    # Load the data
    X, y = load_m2ax_data()
    ...
    for i, seed in enumerate(seeds):
        ...
        # print(cat_cols)
        kernel = gpplus.kernels.LogScaleKernel(gpplus.kernels.CombinedKernel(cat_cols=cat_cols, z_dim=z_dim))
