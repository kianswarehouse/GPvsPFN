...
from load_experimental_data import load_dns_rom_hf_data
...
def DNS_ROM_SF_GPvsPFN(num_folds=defaults.NUM_FOLDS,
        test_size=0.2,
    ...
    if title is None:
        title = f"DNS_ROM_SF_{test_size}test_{num_runs}runs"
    else: 
        title = f"DNS_ROM_SF_{title}_{test_size}test_{num_runs}runs"
    ...
    # Load only high-fidelity DNS ROM data
    print("Loading DNS ROM high-fidelity data...")
    X, y = load_dns_rom_hf_data(print_info=True)
    ...
    for i in range(num_folds):
        ...
        # Standardize data (single-fidelity, no source column)
        ...
        if standardize_X:
            ...
            Xscaler.fit(X_train_gp)
            X_train_gp = Xscaler.transform(X_train_gp)
            X_test_gp = Xscaler.transform(X_test_gp)

        # Normalize the GP data
        if standardize_y:
            Yscaler = gpplus.utils.StandardScaler()
            Yscaler.fit(y_train_gp)
            y_train_mean = Yscaler.mean 
            y_train_std = Yscaler.std
            y_train_normal = Yscaler.transform(y_train_gp)
        else:
            y_train_mean = None
            y_train_std = None
            y_train_normal = y_train_gp

        # Use single-fidelity kernel (no source column)
...
if __name__ == "__main__":
    DNS_ROM_SF_GPvsPFN(num_folds=1, test_size=0.2, num_runs=4, num_epochs=10000, save_path="./results/DNS_ROM/temp")