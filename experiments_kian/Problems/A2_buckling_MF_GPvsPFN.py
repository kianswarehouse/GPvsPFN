...
def buckling_GPvsPFN(num_folds=defaults.NUM_FOLDS,
    ...
    if title is None:
        title = f"buckling_MF_{train_size}Dn_{num_runs}runs_noiseTest{noise_test}_noiseTrain{noise_train}"
    else: 
        title = f"buckling_MF_{title}_{train_size}Dn_{num_runs}runs_noiseTest{noise_test}_noiseTrain{noise_train}"
    ...
    for fold in range(num_folds):  # Show all folds
        ...
        source_counts = [torch.sum(fold_data[:, -1] == i).item() for i in range(len(source_cols))]
