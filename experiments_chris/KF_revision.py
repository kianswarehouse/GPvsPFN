# gp_rbf_powerexp_matern05_multistart_stable_paperfig_kf.py
#
...
#   - GP-based models use:
#         * 5 experimental runs (different data splits / seeds)
#         * 4 hyperparameter initializations (multi-start restarts) per run
...
#   - experimental runs = 5 (seeds 0..4)
#   - GP restart initializations per run = 4
...
#         * Test Accuracy (GP+ models only)
#         * Test Brier Score (GP+ models only)
#   - GP-only Kernel Flows figure:
#         * Kernel Flows (GP+ models only)
#   - GP-only Kernel Flows fraction sweep figures:
#         * one figure per train size, showing KF for 10%, 50%, 90% subsets
...
def _sample_subset_indices(
    ...
    *,
    ...
    for _ in range(int(n_subsamples)):
        inds = torch.randperm(N, device=device, generator=gen)[:Nf]
...
def _quadratic_form_from_operator(
    ...
    *,
    ...
    """
    try:
        K_j = K_op.add_jitter(jitter)
        chol = K_j.cholesky()
        alpha = chol.solve(r.unsqueeze(-1)).squeeze(-1)
        val = (r * alpha).sum()
        return float(val.item())
    except Exception:
        K_dense = K_op.to_dense() if hasattr(K_op, "to_dense") else K_op
        eye = torch.eye(K_dense.size(-1), device=K_dense.device, dtype=K_dense.dtype)
        K_dense = K_dense + jitter * eye
        L = torch.linalg.cholesky(K_dense)
        alpha = torch.cholesky_solve(r.unsqueeze(-1), L).squeeze(-1)
        val = (r * alpha).sum()
        return float(val.item())


def compute_kf_metric_gpclassifier(
    model,
    ...
    *,
    ...
    """
    Kernel Flows (KF) metric for the GPClassifier / Dirichlet-target case.
    Nf defaults to N // 2 if not explicitly provided.
    """
    try:
        ...
        with torch.no_grad(), gpytorch.settings.cholesky_jitter(jitter):
            mean_full = model.mean_module(train_x)
            K_full = model.covar_module(train_x)

            if mean_full.ndim != 2 or train_y_CxN.ndim != 2:
                return float("nan")
            if train_y_CxN.shape != mean_full.shape:
                return float("nan")

            C, N_mean = mean_full.shape
            if N_mean != N:
                return float("nan")

            inds_list = _sample_subset_indices(
                N,
                Nf,
            ...
            for c in range(C):
                r_full = (train_y_CxN[c] - mean_full[c]).view(-1)

                try:
                    K_full_c = K_full[c]
                except Exception:
                    K_full_dense = K_full.to_dense()
                    K_full_c = K_full_dense[c]

                if torch.is_tensor(K_full_c):
                    eye = torch.eye(K_full_c.size(-1), device=K_full_c.device, dtype=K_full_c.dtype)
                    K_full_c = K_full_c + jitter * eye
                    L = torch.linalg.cholesky(K_full_c)
                    alpha = torch.cholesky_solve(r_full.unsqueeze(-1), L).squeeze(-1)
                    u_sq = float((r_full * alpha).sum().item())
                else:
                    u_sq = _quadratic_form_from_operator(K_full_c, r_full, jitter=jitter)

                if u_sq <= 0 or not torch.isfinite(torch.tensor(u_sq, device=train_x.device)):
                    continue

                rhos_sub = []
                for inds in inds_list:
                    x_sub = train_x[inds]
                    mean_sub_all = model.mean_module(x_sub)
                    r_sub = (train_y_CxN[c, inds] - mean_sub_all[c]).view(-1)

                    K_sub_all = model.covar_module(x_sub)
                    try:
                        K_sub_c = K_sub_all[c]
                    except Exception:
                        K_sub_dense = K_sub_all.to_dense()
                        K_sub_c = K_sub_dense[c]

                    if torch.is_tensor(K_sub_c):
                        eye = torch.eye(K_sub_c.size(-1), device=K_sub_c.device, dtype=K_sub_c.dtype)
                        K_sub_c = K_sub_c + jitter * eye
                        L = torch.linalg.cholesky(K_sub_c)
                        alpha = torch.cholesky_solve(r_sub.unsqueeze(-1), L).squeeze(-1)
                        v_sq = float((r_sub * alpha).sum().item())
                    else:
                        v_sq = _quadratic_form_from_operator(K_sub_c, r_sub, jitter=jitter)
                    ...
                    if torch.isfinite(torch.tensor(rho, device=train_x.device)):
                        rhos_sub.append(float(rho))

                if len(rhos_sub) == 0:
                    continue

                rho_classes.append(float(sum(rhos_sub) / len(rhos_sub)))

            if len(rho_classes) == 0:
                return float("nan")

            return float(sum(rho_classes) / len(rho_classes))

    except Exception:
        return float("nan")


def compute_kf_fraction_sweep(
    model,
    train_x: torch.Tensor,
    train_y_CxN: torch.Tensor,
    *,
    ...
    """
    Compute KF for several subset fractions, e.g. 10%, 50%, 90%.
    """
    out = {}
    N = train_x.size(0)

    for frac in fractions:
        key = f"kf_frac_{int(round(100 * frac))}"
        Nf = int(round(frac * N))
        Nf = max(1, min(Nf, N - 1))

        try:
            val = compute_kf_metric_gpclassifier(
                model,
                train_x=train_x,
                train_y_CxN=train_y_CxN,
                Nf=Nf,
...
def train_gp_model_with_restarts(
    *,
    ...
    kf_fraction_sweep: Optional[List[float]] = None,
    ...
    if compute_kf:
        final_kf = compute_kf_metric_gpclassifier(
            best_model,
            train_x=X_t,
            train_y_CxN=y_CxN,
        ...
        if kf_fraction_sweep is not None and len(kf_fraction_sweep) > 0:
            kf_fraction_results = compute_kf_fraction_sweep(
                best_model,
                train_x=X_t,
                train_y_CxN=y_CxN,
                fractions=kf_fraction_sweep,
                cholesky_jitter=best["jitter_used"],
                seed=best["restart_seed"],
                n_subsamples=8,
                max_n=2000,
...
class GPPlusGPClassifierRBF(BinaryModel):
    def __init__(
        ...
        self.kf_fraction_sweep = kf_fraction_sweep if kf_fraction_sweep is not None else [0.10, 0.50, 0.90]
    ...
    def fit_predict(
        ...
        return train_gp_model_with_restarts(
            ...
            kf_fraction_sweep=self.kf_fraction_sweep,
        )
...
def plot_gp_kf_fraction_sweep_by_train_size(
    summary_df: pd.DataFrame,
    out_dir: str,
    title_prefix: Optional[str] = None,
) -> List[str]:
    """
    For each train size, create a separate plot showing KF vs subsample fraction
    (10%, 50%, 90%) for the three GP models.
    """
    if summary_df.empty:
        raise ValueError("summary_df is empty; cannot create KF fraction sweep plots.")
    ...
    frac_labels = ["10%", "50%", "90%"]
    frac_cols = ["kf_frac_10_mean", "kf_frac_50_mean", "kf_frac_90_mean"]
    marker_cycle = ["o", "s", "^"]
    model_to_marker = {m: marker_cycle[i] for i, m in enumerate(gp_models)}

    saved_paths = []

    for train_size in sorted(df["train_size"].dropna().unique()):
        plt.figure(figsize=(6.8, 4.8))
        plotted_any = False

        dft = df[df["train_size"] == train_size].copy()

        for model in gp_models:
            row = dft[dft["model"] == model]
            if row.empty:
                continue
            ...
            for col in frac_cols:
                if col not in row.columns:
                    ...
                    yvals.append(float(row.iloc[0][col]))
        ...
        plt.xlabel("KF Subsample Fraction")
        plt.ylabel("Kernel Flows")
        plt.title(f"Kernel Flows vs Subsample Fraction (Train Size = {train_size})")
        plt.grid(True, alpha=0.25)
        plt.legend(frameon=False)

        if title_prefix:
            plt.suptitle(title_prefix, y=1.02, fontsize=13)

        out_path = os.path.join(out_dir, f"gp_kf_fraction_sweep_train{train_size}.png")
        plt.tight_layout()
        plt.savefig(out_path, dpi=300, bbox_inches="tight")
        plt.close()
            ...
            for model in models:
                ...
                try:
                    ...
                    row = {
                        ...
                        "kf_frac_10": None if trainer_info is None else trainer_info.get("kf_frac_10"),
                        "kf_frac_50": None if trainer_info is None else trainer_info.get("kf_frac_50"),
                        "kf_frac_90": None if trainer_info is None else trainer_info.get("kf_frac_90"),
                        **m,
                    }
                    rows.append(row)
                    print(row)
...
def summarize(df: pd.DataFrame) -> pd.DataFrame:
    return df.groupby(["model", "train_size"], as_index=False).agg(
        ...
        kf_frac_10_mean=("kf_frac_10", "mean"),
        kf_frac_10_std=("kf_frac_10", "std"),
        kf_frac_50_mean=("kf_frac_50", "mean"),
        kf_frac_50_std=("kf_frac_50", "std"),
        kf_frac_90_mean=("kf_frac_90", "mean"),
        kf_frac_90_std=("kf_frac_90", "std"),
...
def main():
    ...
    n_restarts = 4
    kf_subset_size = 32
    kf_fraction_sweep = [0.10, 0.50, 0.90]
    ...
    print(f"KF fraction sweep: {kf_fraction_sweep}")
    ...
    models: List[BinaryModel] = [
        GPPlusGPClassifierRBF(
            ...
            kf_fraction_sweep=kf_fraction_sweep,
        ),
    ...
    raw_path = os.path.join(out_dir, "grid_rbf_matern05_powerexp_multistart_stable_kf_raw.csv")
    summ_path = os.path.join(out_dir, "grid_rbf_matern05_powerexp_multistart_stable_kf_summary.csv")
    ...
    kf_sweep_dir = os.path.join(out_dir, "gp_kf_fraction_sweep_plots")
    ensure_dir(kf_sweep_dir)
    saved_kf_sweep_plots = plot_gp_kf_fraction_sweep_by_train_size(
        summary_df=summ,
        out_dir=kf_sweep_dir,
        title_prefix="Electrical Grid Stability Benchmark",
    )
    print(f"Saved {len(saved_kf_sweep_plots)} KF fraction sweep plots to {kf_sweep_dir}")
