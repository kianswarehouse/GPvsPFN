"""
B4: Ackley. Loads data and bounds from load_experimental_data, sets up runs, calls run_BO.run_BO_gp.
"""
import json
import os
...
import defaults
from load_experimental_data import generate_ackley_data, ackley_function
from run_BO import run_BO_gp, run_BO_pfn, run_BO_Result, get_bounds_ackley
from gpplus.utils import set_seed
from gpplus.utils.metrics_functions import analyze_metrics


def ackley_GPvsPFN_BO(
    num_runs: int = defaults.NUM_RUNS,
    num_test: int = 5000,
    start_size: int = 10,
    dimensions: int = 5,
    x_bounds: list[float] | tuple[float, float] = [-5.0, 10.0],  # same as A4: list or tuple
    ...
    """Ackley: load data from load_experimental_data, get bounds, run BO via run_BO_gp."""
    ...
    v2 = "V2" if V2 else ""
    if title is None:
        title_str = f"{prefix}_BO_Ackley{v2}_{dimensions}Dx_{start_size}Dn_[{x_bounds_tuple[0]},{x_bounds_tuple[1]}]_{num_inits}inits_{max_iter}maxIter_{patience_no_improve}patience_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"
    else:
        title_str = f"{prefix}_BO_Ackley{v2}_{title}_{dimensions}Dx_{start_size}Dn_[{x_bounds_tuple[0]},{x_bounds_tuple[1]}]_{num_inits}inits_{max_iter}maxIter_{patience_no_improve}patience_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"
    ...
    X_train_all, _, X_test_all, y_test_all = generate_ackley_data(
        n_train=total_train,
        n_test=num_test,
    ...
    bounds = get_bounds_ackley(dimensions, x_bounds_tuple, device, dtype)

    def objective_fn_clean(x: torch.Tensor) -> torch.Tensor:
        return ackley_function(x, dimensions=dimensions)
    ...
    print("\n" + "=" * 60)
    print("BO Ackley – Data & model setup")
    print("=" * 60)
    gp_model_info = {
        "title": title_str,
        "dimensions": dimensions,
        ...
        "train_samples_per_run": start_size * dimensions,
    ...
    time_end = time.time()
    print(f"Total BO time for Ackley problem: {time_end - time_start:.2f}s")
...
# Alias for run_GPvsPFN_BO and __main__

if __name__ == "__main__":
    # print("B4 Ackley BO smoke test (max_iter=2)...")
    out = ackley_GPvsPFN_BO(
        num_runs=1, start_size=5, dimensions=10, x_bounds=[-5.0, 10.0],
