"""
B5: Rastrigin. Loads data and bounds from load_experimental_data, sets up runs, calls run_BO.run_BO_gp.
"""
import json
import os
...
def rastrigin_GPvsPFN_BO(
    num_runs: int = defaults.NUM_RUNS,
    num_test: int = 5000,
    start_size: int = 10,
    dimensions: int = 5,
    x_bounds: list[float] | tuple[float, float] = (-5.12, 5.12),
    ...
    minimization_problem: bool = True,  # Rastrigin: minimize objective
    ...
    """Rastrigin: load data from load_experimental_data, get bounds, run BO via run_BO_gp."""
    ...
    if title is None:
        title_str = f"{prefix}_BO_Rastrigin_{dimensions}Dx_{start_size}Dn_[{x_bounds_tuple[0]},{x_bounds_tuple[1]}]_{num_inits}inits_{max_iter}maxIter_{patience_no_improve}patience_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"
    else:
        title_str = f"{prefix}_BO_Rastrigin_{title}_{dimensions}Dx_{start_size}Dn_[{x_bounds_tuple[0]},{x_bounds_tuple[1]}]_{num_inits}inits_{max_iter}maxIter_{patience_no_improve}patience_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"

    num_runs_gen = max(num_runs, 20)
    train_per_run = start_size * dimensions
    total_train = num_runs_gen * train_per_run

    X_train_all, _, X_test_all, y_test_all = generate_rastrigin_data(
    ...
    def objective_fn_clean(x: torch.Tensor) -> torch.Tensor:
        return rastrigin_function(x, dimensions=dimensions)
    ...
    print("\n" + "=" * 60)
    print("BO Rastrigin – Data & model setup")
    print("=" * 60)
    gp_model_info = {
        "title": title_str,
    ...
    time_end = time.time()
    print(f"Total BO time for Rastrigin problem: {time_end - time_start:.2f}s")
...
if __name__ == "__main__":
    print("B5 Rastrigin BO smoke test (max_iter=2)...")
    out = rastrigin_GPvsPFN_BO(
        num_runs=1, num_test=100, start_size=2, dimensions=5, x_bounds=(-5.12, 5.12),
