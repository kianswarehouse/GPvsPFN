...
class PerFeatureTransformer(nn.Module):
    ...
    def forward(self, *args: Any, **kwargs: Any) -> dict[str, torch.Tensor]:  # noqa: D417
        ...
        if len(args) == 3:
            # print(f"Rosen: Here 3")
            # print(f"Rosen have modified")
            style, x, y = args
            # return self._forward(x, y, style=style, **kwargs)
            # return self._forward_Rosen(x, y, style=style, **kwargs)
            # Use the ROSEN flag to determine which forward method to use
            if ROSEN:
                print(f"Rosen trueornot: {ROSEN}")
                return self._forward_Rosen(x, y, style=style, **kwargs)
            else:
                return self._forward(x, y, style=style, **kwargs)
    ...
    def _forward(  # noqa: PLR0912, C901
        ...
        *,
        ...
        # print(f"train_encoder_out: {train_encoder_out.shape}")
        # print(f"test_encoder_out: {test_encoder_out.shape}")
        ...
        # Rosen
        # print(f"output_decoded train_embeddings: {output_decoded['train_embeddings'].shape}")
        # print(f"output_decoded test_embeddings: {output_decoded['test_embeddings'].shape}")
        # print(f"output_decoded: {output_decoded.keys()}")
        # print(f"output_decoded standard: {output_decoded['standard'].shape}")
    ...
    def _forward_Rosen(  # Rosen
        ...
        *,
        ...
        # print(f"Rosen print x: {x['main'].shape}")
        OG_BATCH_SIZE = x['main'].shape[1]
        ...
        # print(f"embedded_y: {embedded_y.unsqueeze(2).shape}")
        ...
        # Rosen
        # print(f"Rosen encoder_out: {encoder_out.shape}")
        ...
        # Rosen's mod: change test to train
        test_encoder_out = encoder_out[:, :single_eval_pos_, -1].transpose(0, 1)
        print(f"Rosen train trans encoded out: {test_encoder_out.shape}")
        
        # test_encoder_out = self.my_pooling_layer(test_encoder_out)
        # print(f"Rosen test_encoder_out: {test_encoder_out.shape}")

        if only_return_standard_out:
            assert self.decoder_dict is not None
            # output_decoded = self.decoder_dict["standard"](test_encoder_out)
            # Rosen
            output_decoded = self.decoder_dict["standard"](train_encoder_out)
        else:
            output_decoded = (
                {k: v(test_encoder_out) for k, v in self.decoder_dict.items()}
                if self.decoder_dict is not None
...
class CrossAttentionPool(nn.Module):
    def __init__(self, hidden_dim, num_heads=8):
        super().__init__()
        # A single learnable query token of shape [1,1,D].
        #   - 1 for "sequence length" of the query
        #   - 1 for batch dimension placeholder
        #   - D for hidden size
        self.query = nn.Parameter(torch.randn(1, 1, hidden_dim))
        
        # Multi-Head Attention expects shape [seq_len, batch, embed_dim]
        # We'll treat 'query' as [1, B, D], 
        # and the keys/values as [N, B, D].
        self.attn = nn.MultiheadAttention(hidden_dim, num_heads, batch_first=False)

    def forward(self, x):
        """
        x: Tensor of shape [N, B, D], i.e. N tokens, B batch, D hidden.
        return: Pooled tensor of shape [1, B, D].
        """
        N, B, D = x.shape
        
        # 1) Expand our single query to batch size B
        #    so shape becomes [1, B, D].
        query = self.query.expand(-1, B, -1)  # same D, same "seq length"=1, but repeated for B

        # 2) MultiheadAttention in PyTorch has the signature:
        #    attn(query, key, value), all shape [S, B, E].
        #    We'll use x as key & value, shape [N, B, D].
        #    We'll get out shape [1, B, D] for the single query.
        out, attn_weights = self.attn(query, x, x)

        # out: [1, B, D] is the "pooled" representation
        # attn_weights: [B, 1, N] are the attention weights per query token
        return out  # [1, B, D]
...
class AttentionPooling(nn.Module):
    def __init__(self, hidden_dim):
        super().__init__()
        # A simple linear layer to produce a single "score" for each vector h_i
        self.attn = nn.Linear(hidden_dim, 1, bias=False)

    def forward(self, x):
        """
        x: Tensor of shape [N, B, D]
        Returns: Tensor of shape [1, B, D]
        """
        # 1) Compute raw scores: shape [N, B, 1]
        scores = self.attn(x)
        
        # 2) Convert scores -> weights using softmax across dim=0 (the N dimension)
        weights = torch.softmax(scores, dim=0)  # shape [N, B, 1]
        
        # 3) Multiply each h_i by its attention weight, then sum over N
        weighted_sum = (x * weights).sum(dim=0, keepdim=True)  # shape [1, B, D]
        return weighted_sum
