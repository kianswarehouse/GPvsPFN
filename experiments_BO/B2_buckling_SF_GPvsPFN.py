"""
B2: Buckling SF. Loads data and bounds from load_experimental_data, sets up runs, calls run_BO.run_BO_gp.
"""
import json
import os
...
import defaults
import gpplus
from load_experimental_data import generate_mf_buckling_data, buckling_mixed_variables
from run_BO import run_BO_gp, run_BO_pfn, run_BO_Result, get_bounds_buckling
...
def buckling_SF_GPvsPFN_BO(
    ...
    save_path: str | None = "./results/buckling",
    title: str | None = None,
    ...
    """Buckling SF: load data from load_experimental_data, get bounds, run BO via run_BO_gp."""
    ...
    if title is None:
        title_str = f"{prefix}_BO_buckling_SF_{start_size}Dn_{num_inits}inits_{max_iter}maxIter_{patience_no_improve}patience_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"
    else:
        title_str = f"{prefix}_BO_buckling_SF_{title}_{start_size}Dn_{num_inits}inits_{max_iter}maxIter_{patience_no_improve}patience_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"
    ...
    X_train_all, _, X_test_all, y_test_all = generate_mf_buckling_data(
    ...
    X_train_all, cont_cols, cat_cols, source_cols = encode_qual_data(
        X_train_all, qual_dict=qual_dict, source_col=None
    )
    X_test_all, _, _, _ = encode_qual_data(
        X_test_all, qual_dict=qual_dict, source_col=None
    )
    ...
    # Build bounds in encoded space: continuous columns use physical ranges, encoded one-hots in [0, 1].
    encoded_dim = X_train_all.shape[1]
    bounds = torch.zeros((2, encoded_dim), device=device, dtype=dtype)
    bounds[1] = 1.0
    # For buckling we have a single continuous variable L with bounds [0.5, 1.5].
    for c in cont_cols:
    ...
    def decode_encoded_to_phys(x: torch.Tensor) -> torch.Tensor:
        """Decode a single encoded x (or batch) to physical (L, E, K, I) for logging."""
        x = x.to(device=device, dtype=dtype)
        if x.dim() == 1:
            x = x.unsqueeze(0)
        # Continuous L
        L_vals = x[:, cont_cols[0]]
        # Categorical groups for E, K, I
        E_onehot = x[:, cat_cols[0]]
        K_onehot = x[:, cat_cols[1]]
        I_onehot = x[:, cat_cols[2]]
        E_idx = torch.argmax(E_onehot, dim=-1)
        ...
        return X_phys.squeeze(0)

    # Data & model setup (print once, like B1/B4)
    print("\n" + "=" * 60)
    print("BO Buckling SF – Data & model setup")
    print("=" * 60)
    gp_model_info = {
        "title": title_str,
        "input_dim": dim,
    ...
    for run_idx in range(num_runs):
        ...
        # Mixed-variable kernel: continuous columns + grouped one-hot categorical columns.
    ...
    time_end = time.time()
    print(f"Total BO time for Buckling problem: {time_end - time_start:.2f}s")
...
if __name__ == "__main__":
    print("B2 Buckling BO smoke test (max_iter=2)...")
    out = buckling_SF_GPvsPFN_BO(num_runs=1, num_test=5000, start_size=5, max_iter=10, patience_no_improve=10, run_models="gp", noise_train=0.08, noise_test=0.08)
