...
OUT_DIR = SCRIPT_DIR / "results" / "compare_BO"
...
def load_result_file(path: Path) -> list[tuple[int, list[float], float | None]] | None:
    """
...
def collect_runs(model_dir: Path) -> tuple[dict[tuple[str, str, str], list[RunTuple]], dict[tuple[str, str, str], str]]:
    """
...
def plot_problem_comparison(
    ...
    """Plot best y vs iteration for one problem across N models. Solid = average best_y; dashed = individual runs."""
    ...
    for idx, (label, runs) in enumerate(models):
        ...
        # Individual runs: dashed
        for run_id, hist, _ in runs:
            iters = list(range(1, len(hist) + 1))
            ax.plot(iters, hist, color=color, alpha=0.5, linestyle="--")

        # Solid line: average best_y at each iteration
        iters, mean_vals = _average_best_y_at_each_iteration(runs)
        if iters and not np.all(np.isnan(mean_vals)):
            ax.plot(iters, mean_vals, color=color, linestyle="-", linewidth=2, label=label)
...
#   DEFAULT_PATHS = [
#       ("results/GP+", "GP+"),
#       ("results/PFN_V2.0_no_GI", "PFN 2.0"),
#       ("results/PFN_V2.0_GI", "PFN 2.0_GI"),
#       ("results/PFN_V2.5_no_GI", "PFN 2.5"),
