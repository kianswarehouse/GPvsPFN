"""
B9: Dixon-Price. Loads data and bounds from load_experimental_data, sets up runs, calls run_BO.run_BO_gp.
"""
import json
import os
...
import defaults
from load_experimental_data import generate_dixon_price_data, dixon_price_function
...
def dixon_price_GPvsPFN_BO(
    num_runs: int = defaults.NUM_RUNS,
    num_test: int = 5000,
    start_size: int = 10,
    dimensions: int = 5,
    x_bounds: list[float] | tuple[float, float] = (-10.0, 10.0),
    ...
    save_path: str | None = "./results/dixon_price",
    title: str | None = None,
    ...
    minimization_problem: bool = True,  # Dixon-Price: minimize objective
    ...
    """Dixon-Price: load data from load_experimental_data, get bounds, run BO via run_BO_gp."""
    ...
    if title is None:
        title_str = f"{prefix}_BO_Dixon_Price_{dimensions}Dx_{start_size}Dn_[{x_bounds_tuple[0]},{x_bounds_tuple[1]}]_{num_inits}inits_{max_iter}maxIter_{patience_no_improve}patience_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"
    else:
        title_str = f"{prefix}_BO_Dixon_Price_{title}_{dimensions}Dx_{start_size}Dn_[{x_bounds_tuple[0]},{x_bounds_tuple[1]}]_{num_inits}inits_{max_iter}maxIter_{patience_no_improve}patience_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"

    num_runs_gen = max(num_runs, 20)
    train_per_run = start_size * dimensions
    total_train = num_runs_gen * train_per_run

    X_train_all, _, X_test_all, y_test_all = generate_dixon_price_data(
    ...
    def objective_fn_clean(x: torch.Tensor) -> torch.Tensor:
        return dixon_price_function(x, dimensions=dimensions)
    ...
    print("\n" + "=" * 60)
    print("BO Dixon-Price – Data & model setup")
    print("=" * 60)
    gp_model_info = {
        "title": title_str,
    ...
    time_end = time.time()
    print(f"Total BO time for Dixon-Price problem: {time_end - time_start:.2f}s")
...
if __name__ == "__main__":
    print("B9 Dixon-Price BO smoke test (max_iter=2)...")
    out = dixon_price_GPvsPFN(
        num_runs=1, num_test=100, start_size=2, dimensions=5, x_bounds=(-10.0, 10.0),
