"""
B7: Zakharov. Loads data and bounds from load_experimental_data, sets up runs, calls run_BO.run_BO_gp.
"""
import json
import os
...
def zakharov_GPvsPFN_BO(
    ...
    save_path: str | None = "./results/zakharov",
    title: str | None = None,
    ...
    minimization_problem: bool = True,  # Zakharov: minimize objective
    ...
    """Zakharov: load data from load_experimental_data, get bounds, run BO via run_BO_gp."""
    ...
    if title is None:
        title_str = f"{prefix}_BO_Zakharov_{dimensions}Dx_{start_size}Dn_[{x_bounds_tuple[0]},{x_bounds_tuple[1]}]_{num_inits}inits_{max_iter}maxIter_{patience_no_improve}patience_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"
    else:
        title_str = f"{prefix}_BO_Zakharov_{title}_{dimensions}Dx_{start_size}Dn_[{x_bounds_tuple[0]},{x_bounds_tuple[1]}]_{num_inits}inits_{max_iter}maxIter_{patience_no_improve}patience_noiseTest{noise_test}_noiseTrain{noise_train}_x{num_runs}"

    num_runs_gen = max(num_runs, 20)
    train_per_run = start_size * dimensions
    total_train = num_runs_gen * train_per_run

    X_train_all, _, X_test_all, y_test_all = generate_zakharov_data(
    ...
    def objective_fn_clean(x: torch.Tensor) -> torch.Tensor:
        return zakharov_function(x, dimensions=dimensions)
    ...
    print("\n" + "=" * 60)
    print("BO Zakharov – Data & model setup")
    print("=" * 60)
    gp_model_info = {
        "title": title_str,
    ...
    time_end = time.time()
    print(f"Total BO time for Zakharov problem: {time_end - time_start:.2f}s")
...
if __name__ == "__main__":
    print("B7 Zakharov BO smoke test (max_iter=2)...")
    out = zakharov_GPvsPFN_BO(
